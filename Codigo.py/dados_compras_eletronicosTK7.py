...
Este programa foi desenvolvido para gerar dados fictícios de compras eletrônicas, permitindo a criação de planilhas personalizadas com informações como clientes, produtos, métodos de pagamento, localização e status de transações. Abaixo estão as funcionalidades e instruções de uso:

---

### **1. Geração de Dados Fictícios**
O programa gera dados fictícios de compras eletrônicas, incluindo:
- **ID do Cliente** e **Nome do Cliente**.
- **Email** do cliente (gerado automaticamente com base no nome).
- **ID do Produto** e **Nome do Produto** (eletrônicos).
- **Data da Compra** (dentro de um intervalo definido).
- **Valor do Produto** e valores ajustados para diferentes métodos de pagamento (à vista, Pix, cartão, boleto).
- **Nome da Loja** e **Nome do Vendedor**.
- **Localização**: País, estado e cidade da compra.
- **Método de Pagamento** e **Status da Transação**.

---

### **2. Configurações de Geração de Dados**
- **Número de Linhas**:
  - Defina quantas linhas de dados deseja gerar.
- **Porcentagens de Acréscimo**:
  - Ajuste as porcentagens de acréscimo para os métodos de pagamento:
    ...
    - Boleto.
- **Formato dos IDs**:
  - Defina o formato dos IDs do cliente e do produto (por exemplo, `ABC123` ou `XY-456`).
- **Intervalo de Datas**:
  - Escolha a data inicial e final para as compras (formato `DD/MM/AAAA`).
- **Seleção de Colunas**:
  - Escolha quais colunas serão incluídas na planilha.
- **Seleção de Países**:
  - Selecione os países de origem das compras (Brasil, Estados Unidos, França, Alemanha, China, Índia, Rússia, Austrália).
- **Personalização de Produtos**:
  - Adicione ou remova produtos da lista de produtos eletrônicos.

---

### **3. Personalização de Dados**
O programa permite personalizar várias listas de dados:
- **Produtos**:
  - Adicione ou remova produtos eletrônicos.
- **Lojas**:
  - Adicione ou remova nomes de lojas.
- **Vendedores**:
  - Adicione ou remova nomes de vendedores.
- **Clientes**:
  - Adicione ou remova nomes de clientes.
- **Domínios de Email**:
  - Adicione ou remova domínios de email (por exemplo, `gmail.com`, `hotmail.com`).
- **Países, Estados e Cidades**:
  - Adicione ou remova países, estados e cidades para personalizar a localização das compras.

---

### **4. Exportação de Dados**
- **Salvar como Excel (`xlsx`)**:
  - Exporta os dados gerados para um arquivo Excel.
- **Salvar como CSV**:
...
- **Tabela de Visualização**:
  - Os dados gerados são exibidos em uma tabela interativa.
- **Ajuste de Colunas**:
  - Ajuste automaticamente a largura das colunas para melhor visualização.
- **Salvar em Bloco de Notas**:
  - Exporte os dados visualizados para um arquivo de texto (`txt`).
- **Número da Linha**:
  - A tabela de visualização inclui uma coluna com o número da linha para facilitar a referência.

---

### **6. Interface Gráfica**
- **Barra de Progresso**:
  - Mostra o progresso da geração de dados.
- **Cronômetro**:
  - Exibe o tempo decorrido durante a geração de dados.
- **Menu de Contexto**:
  - Clique com o botão direito para acessar opções como "Copiar" e "Limpar".
- **Marcar/Desmarcar Colunas e Países**:
  - Botões para marcar ou desmarcar todas as colunas ou países de uma vez.
- **Personalização de Produtos**:
  - Adicione ou remova produtos da lista de produtos eletrônicos.
...
- **Validações**:
  - O programa valida entradas inválidas, como números de linhas negativos, datas fora do formato correto ou porcentagens inválidas.
- **Threading**:
  - A geração de dados e exportação são realizadas em threads separadas para evitar travamentos na interface.
- **Personalização**:
  - O programa é altamente personalizável, permitindo ajustar diversos parâmetros para atender às suas necessidades.
...
1. Na aba **Configurações**, defina os parâmetros desejados:
   - Número de linhas.
   - Porcentagens de acréscimo para métodos de pagamento.
   - Formato dos IDs do cliente e do produto.
   - Intervalo de datas.
   - Colunas e países desejados.
   - Adicione ou remova produtos da lista de produtos eletrônicos.
2. Clique em **Visualizar Dados** para gerar e visualizar os dados.
3. Use os botões **Salvar como Excel** ou **Salvar como CSV** para exportar os dados.
4. Na janela de visualização:
   - Ajuste as colunas automaticamente com o botão **Ajustar Colunas**.
   - Salve os dados em um arquivo de texto com o botão **Salvar em Bloco de Notas**.
   
Conclusão
Este programa é uma ferramenta poderosa para gerar dados fictícios de compras eletrônicas, com uma interface gráfica amigável e várias opções de personalização. Ele pode ser útil para testes, simulações ou qualquer situação em que você precise de dados fictícios para análise ou desenvolvimento.

---

###  Agradecimento**
Obrigado por utilizar este programa! Desenvolvido com carinho por **Diogo Centeno**.  
