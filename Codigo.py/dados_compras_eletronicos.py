...
# Inicializando Faker e semente para reprodutibilidade
fake = Faker()
...
for i in tqdm(range(num_linhas), desc="Gerando dados"):
    ...
    transacao = random.choice(["À vista", "Cartão", "Boleto"])

    # Definição do status
    if transacao == "À vista":
        status = "Aprovado"
    else:
        status = random.choice(["Aprovado", "Recusado", "Error"])

    dados.append([
        f"{''.join(random.choices(string.ascii_uppercase, k=3))}{i + 1}",
        fake.name(),
        ...
        locale.currency(preco * 1.05, grouping=True),
        locale.currency(preco * 1.10, grouping=True),
        random.choice(lojas),
        fake.name(),
        pais,
        estado,
...
df = pd.DataFrame(dados, columns=[
    ...
    'País da Compra', 'Estado da Compra', 'Cidade da Compra', 'Transação', 'Status'
])
